...
import { useMigrateImagesMutation, useGetMyPostQuery, useDeletePostMutation, useCreatePostMutation } from '@/redux/slices/apiSlice';
...
const EditPostPage = () => {
  ...
  const {id} = useParams();
  const {data:editPostData} = useGetMyPostQuery(id,{skip: !id});
  const [deletePost, {error: deletePostError}] = useDeletePostMutation();
  const [migrateImages] = useMigrateImagesMutation();
  ...
  useEffect(() => {
    if (editPostData && editPostData[0]) {
      console.log('Post data received:', editPostData[0]);
      const data = editPostData[0];
      setSelectedMake(data.brand || '');
      setModel(data.model || '');
      setYear(data.year || '');
      setPrice(data.price || '');
      ...
      setBody(data.body || '');
      setNumberOfDoors(data.number_of_doors || '');
      ...
      if (data.brand) {
        handleMakeSelection(data.brand);
      }
    }
  }, [editPostData]);
  ...
  useEffect(() => {
    if (editPostData?.[0]?.image_urls) {
      setImages(editPostData[0].image_urls);
    }
  }, [editPostData]);
  ...
  const handleEditPost = async (e) => {
    ...
    const allImages = [...existingImageFiles, ...newImageFiles];
    ...
    try {
      ...
      setErrors(prev => ({ ...prev, general: "Failed to edit post. Please try again later." }));
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <form className="space-y-8" onSubmit={handleEditPost}>
        <Card>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <ImageUploadComponent 
              setImages={setImages} 
              editSetImages={images}
              onImageDelete={(deletedImage) => {
        ...
        <button
          type='submit'
          className={`bg-red-500 text-white text-lg font-bold py-2 px-24 ${
