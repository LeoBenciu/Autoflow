...
describe('LoginForm Component', () => {
  it('renders the login form correctly', () => {
    ...
    const submitLoginButton = buttons.find(
      (button) =>
        button.textContent === 'Login' && button.getAttribute('type') === 'submit'
    );
  ...
  it('submits the form successfully', async () => {
    const resolvedData = { data: { id: 1, name: 'John Snow', email: 'test@example.com' } };

    const mockLogin = jest.fn(() =>
      Promise.resolve({
        unwrap: () => Promise.resolve(resolvedData),
      })
    );

    jest
      .spyOn(require('@/redux/slices/apiSlice'), 'useLoginMutation')
      .mockReturnValue([mockLogin, { isLoading: false }]);
